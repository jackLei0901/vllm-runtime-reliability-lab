"""Two-rank FSDP2 reproducer for mixed dtypes with an unused parameter.

Run only through ``run_campaign.py`` so process cleanup and result redaction are
    ...
    try:
        ...
        if rank == 0:
            print(
                "DFX_RESULT="
                + json.dumps(
                    {"arm": args.arm, "completed_steps": args.steps},
                    sort_keys=True,
                ),
                flush=True,
            )
