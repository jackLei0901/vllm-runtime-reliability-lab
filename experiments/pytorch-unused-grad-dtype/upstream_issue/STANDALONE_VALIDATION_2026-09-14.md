...
Status: **PyTorch 2.13 reproduces; 2026-09-13 nightly does not reproduce**
...
therefore this is not the Gate 1g missing-responder stall.
...
the draft reproduces the PyTorch 2.13 behavior. The nightly result also changes
the upstream action. This should not be filed as an unresolved nightly bug.
Before opening an issue, determine whether the relevant change is already known
and whether the maintained 2.13 line is eligible for a backport. The experiment
does not identify which PyTorch or NCCL change removed the stall.
