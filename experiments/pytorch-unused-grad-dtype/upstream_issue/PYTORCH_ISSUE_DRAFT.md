# [c10d] Flight Recorder misses a rank blocked in legacy ProcessGroupNCCL teardown
...
On current nightly, legacy `ProcessGroupNCCL::shutdown()` stops the heartbeat
monitor
([source](https://github.com/pytorch/pytorch/blob/13376c2070a764e25f67b2385c31358b325e8a1c/torch/csrc/distributed/c10d/ProcessGroupNCCL.cpp#L1592))
before destroying NCCL communicators
([source](https://github.com/pytorch/pytorch/blob/13376c2070a764e25f67b2385c31358b325e8a1c/torch/csrc/distributed/c10d/ProcessGroupNCCL.cpp#L1594-L1602)).
That monitor is also the thread that polls the store for `exception_dump`
([source](https://github.com/pytorch/pytorch/blob/13376c2070a764e25f67b2385c31358b325e8a1c/torch/csrc/distributed/c10d/ProcessGroupNCCL.cpp#L1865-L1929)).
The same ordering exists in v2.13.0
([stop](https://github.com/pytorch/pytorch/blob/v2.13.0/torch/csrc/distributed/c10d/ProcessGroupNCCL.cpp#L1582),
[destroy](https://github.com/pytorch/pytorch/blob/v2.13.0/torch/csrc/distributed/c10d/ProcessGroupNCCL.cpp#L1585-L1590),
[poll](https://github.com/pytorch/pytorch/blob/v2.13.0/torch/csrc/distributed/c10d/ProcessGroupNCCL.cpp#L1855-L1919)).
...
Save the following as `reproducer.py`:
...
Nightly's default `nccl` backend is `nccl2`, which does not stall in this
reproducer. Set `TORCH_DIST_USE_NCCL2=0` to select the affected legacy backend.
Run under an external hard bound after removing old dump files:
...
Current nightly `2.15.0.dev20260913+cu130` (git
`13376c2070a764e25f67b2385c31358b325e8a1c`) with NCCL 2.30.7 and
`TORCH_DIST_USE_NCCL2=0` reproduced the missing-rank behavior:
...
The same behavior reproduced on PyTorch `2.13.0+cu130` (git
`cf30153c4c131c8164ee7798e5022d810682e2cb`) with NCCL 2.29.7. Its corresponding
timeout was 30005 ms and its only dump was the 1,428-byte rank-0 file.

With nightly's default `nccl2` backend, rank 1's
`destroy_process_group()` returns and the job exits in about 35 seconds; that
backend was not investigated.
...
- PyTorch nightly: `2.15.0.dev20260913+cu130`
...
- Python: 3.12.3
- release corroboration: `2.13.0+cu130`, git
  `cf30153c4c131c8164ee7798e5022d810682e2cb`, NCCL 2.29.7

### Nightly `python -m torch.utils.collect_env`

This command was run from the pip nightly environment used for the reproduction.
The conda section reports unrelated packages in the base environment; the
`[pip3] torch==2.15.0.dev20260913+cu130` entry is the environment that ran it.

```text
Collecting environment information...
PyTorch version: 2.15.0.dev20260913+cu130
...
[pip3] torch==2.15.0.dev20260913+cu130
...
### PyTorch 2.13 `python -m torch.utils.collect_env`
