"""Bounded, PID-reuse-safe lifecycle helpers with exit-race handling.

Version 1 remains byte-frozen by the Gate 1b-1g evidence manifests. This version
treats ESRCH after a successful identity check as a benign concurrent exit.
"""
...
def _signal_if_same(
    ...
    try:
        ...
        # The process may exit after the identity check and before the signal.
        return False
    return True


def _signal_group_if_same(
    identity: ProcessIdentity,
    requested_signal: int,
    proc_root: Path,
) -> bool:
    ...
    try:
        ...
        # The process group may disappear after the identity check.
        return False
    return True
...
def cleanup_process_group(
    ...
    term_group_sent = _signal_group_if_same(parent_identity, TERM_SIGNAL, proc_root)
    try:
    ...
    kill_group_sent = _signal_group_if_same(parent_identity, KILL_SIGNAL, proc_root)
    try:
        parent.wait(timeout=grace_seconds)
    except subprocess.TimeoutExpired:
        pass
