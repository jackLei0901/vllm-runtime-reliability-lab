...
```text
participant present + peer dump request exists + dump responder stopped
...
| shutdown flags show responder stopped, communicator destruction started, and destruction did not complete | primary for producer lifecycle | the expected responder was unavailable while teardown remained incomplete | communicator destruction caused the original application failure; every backend/version has the same lifecycle | source/version pin, ordered stage flags, rank identity | responder observed the later request, destruction completed before the request, or flag order is invalid | producer-loss mechanism is `insufficient_evidence` |
